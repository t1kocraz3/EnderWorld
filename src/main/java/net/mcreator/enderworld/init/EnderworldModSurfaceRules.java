...
import net.mcreator.enderworld.mixin.NoiseGeneratorSettingsAccess;

@EventBusSubscriber
public class EnderworldModSurfaceRules {
	@SubscribeEvent
	public static void init(ServerAboutToStartEvent event) {
		...
		boolean hasEndBiomes = chunkGenerator.getBiomeSource().possibleBiomes().stream().anyMatch(biomeHolder -> biomeHolder.unwrapKey().orElseThrow().location().getNamespace().equals("enderworld"));
		if (hasEndBiomes) {
			if (chunkGenerator instanceof NoiseBasedChunkGenerator generator) {
				...
				registerSurfaceRules(ResourceLocation.parse("enderworld:end_forest"), noiseGeneratorSettings, EnderworldModBlocks.ENDER_TURF.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
				registerSurfaceRules(ResourceLocation.parse("enderworld:shulker_garden"), noiseGeneratorSettings, Blocks.END_STONE.defaultBlockState(), Blocks.END_STONE.defaultBlockState());
				registerSurfaceRules(ResourceLocation.parse("enderworld:endermite_meadow"), noiseGeneratorSettings, EnderworldModBlocks.ENDER_DIRT.get().defaultBlockState(), EnderworldModBlocks.ENDER_DIRT.get().defaultBlockState());
				registerSurfaceRules(ResourceLocation.parse("enderworld:end_forest"), noiseGeneratorSettings, EnderworldModBlocks.ENDER_TURF.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
				registerSurfaceRules(ResourceLocation.parse("enderworld:end_forest"), noiseGeneratorSettings, EnderworldModBlocks.ENDER_TURF.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
			}
		}
